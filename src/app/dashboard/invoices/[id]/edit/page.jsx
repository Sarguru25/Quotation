...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [custRes, itemsRes, taxesRes, invRes] = await Promise.all([
        ...
        if (invData.success && invData.data) {
          ...
          // Zoho prevents editing items if fully paid, but let's allow basic form population
          setForm({
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/zoho/invoices/${id}`, {
        method: "PUT",
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href={`/dashboard/invoices/${id}`} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
            <ArrowLeft size={20} />
          </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                readOnly
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-slate-100 text-slate-500 outline-none cursor-not-allowed"
              />
            </div>
            <div>
