...
export default function ZreqtTab({ canManage }) {
  ...
  const handleSeed = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/zreqt-prices/seed", { method: "POST" });
      const json = await res.json();
      if (json.success) {
        alert(json.message);
        fetchData();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/zreqt-prices/${id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      voltage: editingItem ? editingItem.voltage : [],
    };

    try {
      const url = editingItem ? `/api/zreqt-prices/${editingItem._id}` : `/api/zreqt-prices`;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mt-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900 tracking-tight">ZREQT Actuators</h2>
