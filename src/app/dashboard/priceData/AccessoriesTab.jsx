...
import { Plus, Edit2, Trash2, Database, RefreshCw, X, Search } from "lucide-react";

export default function AccessoriesTab({ canManage }) {
  ...
  const handleSeed = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/accessory-prices/seed", { method: "POST" });
      const json = await res.json();
      if (json.success) {
        alert(json.message);
        fetchData();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/accessory-prices/${id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      specification: editingItem ? editingItem.specification : {},
    };

    try {
      const url = editingItem ? `/api/accessory-prices/${editingItem._id}` : `/api/accessory-prices`;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mt-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900 tracking-tight">Accessory Price Data</h2>
          <p className="text-sm text-gray-500 mt-1">Manage pricing for actuator accessories</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <div className="relative w-full md:w-96">
          ...
          <input type="text" placeholder="Search by model or description..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-sm" />
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading && (data?.length || 0) === 0 ? (
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">No data found. Click "Seed DB" to load from JSON.</td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h2 className="text-xl font-bold text-gray-900">{editingItem ? `Edit Accessory` : `Add New Accessory`}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="mb-4">
                ...
                <input type="text" name="description" value={formData.description} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" />
              </div>

              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">List Price (INR)</label>
                  <input type="number" name="price_inr" value={formData.price_inr} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">List Price (USD)</label>
                  <input type="number" name="price_usd" value={formData.price_usd} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" />
                </div>
              </div>
